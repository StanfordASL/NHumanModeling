...
class SpatioTemporalGraphCVAE(object):
    def __init__(self, pos_dict, robot_node, 
                 edge_radius=5.0, 
        ...
        all_nodes, edge_types, node_edges_and_neighbors = self.get_st_graph_info(pos_dict)
        ...
        instance_connected_to_robot = {node: False for node in all_nodes}
        for node in all_nodes:
            for edge_type in edge_types[node]:
        ...
        for node in all_nodes:
            if node != robot_node:
                self.node_model_dict[node] = MultimodalGenerativeCVAE(node, robot_node,
                                                                      node_edges_and_neighbors[node], 
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            nll_exact = tf.identity(tf.reduce_mean(tf.stack(nll_exact_values), axis=0), 
                                    name=nll_exact_name)
